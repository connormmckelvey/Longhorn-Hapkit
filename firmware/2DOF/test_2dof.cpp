...
void loop() {
  ...
  if(Serial.available()){
    ...
    if (s == '1') hapticMode = JOYSTICK;
    if (s == '2') hapticMode = GRID;
    if (s == '3') hapticMode = CIRCLES;
    if (s == '4') hapticMode = HARP;
    if (s == '5') hapticMode = DAMP;
    if (s == '6') hapticMode = WALL;
    if (s == '7') hapticMode = JOYSTICK_DAMPED;
  }
...
void FK(float theta1, float theta5){
  ...
  float p2p4 = dist(p4x,p4y,p2x,p2y);
  
  float p2ph = (a2*a2-a3*a3+p2p4*p2p4)/(2*p2p4);
  float p3ph = sqrt(a2*a2-p2ph*p2ph);

  float   phx = p2x+(p2ph/p2p4)*(p4x-p2x);
  float phy = p2y+(p2ph/p2p4)*(p4y-p2y);

  float X = phx+(p3ph/p2p4)*(p4y-p2y);
  float Y = phy-(p3ph/p2p4)*(p4x-p2x);
...
void Jac(){
  float d = dist(p2x,p2y,p4x,p4y);
  float b = dist(p2x,p2y,phx,phy);
  float h = dist(X,Y,phx,phy);
